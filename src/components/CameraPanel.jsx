...
export default function CameraPanel({
  ...
  async function startCam() {
    ...
    if (pythonMode) {
      ...
      if (!pythonServerOk) {
        toast.error('Python Vision Server is not running! Run: npm run install:python then restart npm run dev');
        return;
      }
      setStreamOn(true);
      return;
    ...
    if (pythonMode) {
      ...
      const pollId = setInterval(async () => {
        ...
        try {
          ...
          } catch (err) { console.error('[PyServo] Jog failed:', err); isJoggingRef.current = false; }
  ...
  function solveNow() {
    ...
    if (!Hm) { toast.warning("Need at least 4 valid pairs to solve."); return; }
    setH(Hm);
  }
