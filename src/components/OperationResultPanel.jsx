...
import './OperationResultPanel.css';

export default function OperationResultPanel({ jobStage, jobStatistics, operationReport, operationFailure, onOpen }) {
  const success = jobStage === 'finished' && !!operationReport;
  const failure = !success && !!operationFailure;
  const active = Boolean(jobStage && jobStage !== 'idle');
  const status = success ? 'SUCCESS' : failure ? 'FAILURE' : active ? 'IN PROGRESS' : 'NO RESULT';
  const tone = success ? 'success' : failure ? 'error' : active ? 'info' : 'neutral';
